...
namespace ScSyncr.Console.CommandLine
{
    internal class CommandLineApplication
    {
            ...
            for (var index = 0; index < args.Length; index++)
            {
                ...
                if (!processed && option == null)
                {
                    ...
                    if (longOption != null)
                    {
                        ...
                        option =
                            command.Options.SingleOrDefault(
                                opt => string.Equals(opt.LongName, longOption[0], StringComparison.Ordinal));
                        ...
                        if (longOption.Length == 2)
                        {
                            if (!option.TryParse(longOption[1]))
                            {
                                ...
                                throw new Exception(string.Format(
                                    "TODO: Error: unexpected value '{0}' for option '{1}'", longOption[1],
                                    option.LongName));
                            }
                            option = null;
                        }
                    ...
                    if (shortOption != null)
                    {
                        ...
                        option =
                            command.Options.SingleOrDefault(
                                opt => string.Equals(opt.ShortName, shortOption[0], StringComparison.Ordinal));
                        ...
                        if (option == null)
                        {
                            option =
                                command.Options.SingleOrDefault(
                                    opt => string.Equals(opt.SymbolName, shortOption[0], StringComparison.Ordinal));
                        }
                        ...
                        if (shortOption.Length == 2)
                        {
                            if (!option.TryParse(shortOption[1]))
                            {
                                command.ShowHint();
                                throw new Exception(string.Format(
                                    "TODO: Error: unexpected value '{0}' for option '{1}'", shortOption[1],
                                    option.LongName));
                            }
                            option = null;
                        }
                ...
                if (!processed && option != null)
                {
                    ...
                    {
                        command.ShowHint();
                        throw new Exception(string.Format("TODO: Error: unexpected value '{0}' for option '{1}'", arg,
                            option.LongName));
                    }
                    option = null;
                }
        ...
        public void ShowHint()
        {
            if (OptionHelp != null)
            {
                System.Console.WriteLine(string.Format("Specify --{0} for a list of available options and commands.",
                    OptionHelp.LongName));
            }
        }
        ...
        public void ShowHelp(string commandName = null)
        {
            ...
            if (target.Commands.Any())
            {
                ...
                {
                    commandsBuilder.AppendLine();
                    commandsBuilder.AppendFormat("Use \"{0} help [command]\" for more information about a command.",
                        Name);
                    commandsBuilder.AppendLine();
                }
            }
            ...
            System.Console.Write("{0}{1}{2}{3}{4}", versionAndName, headerBuilder, argumentsBuilder, optionsBuilder,
                commandsBuilder);
        }
        ...
        public void ShowVersion()
        {
            ...
            System.Console.WriteLine(VersionGetter());
        }

        private bool HasHelpCommand()
        {
            var helpCmd =
                Commands.SingleOrDefault(cmd => string.Equals("help", cmd.Name, StringComparison.OrdinalIgnoreCase));
            return helpCmd != null;
        }
